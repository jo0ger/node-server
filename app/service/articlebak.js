...
module.exports = class ArticleService extends ProxyService {
    ...
    async update () {
        ...
        return await this.updateItemById(params.id, body).populate('category tag').exec()
    }
    ...
    async updateCommentCount (articleIds = []) {
        ...
        Promise.all(
            counts.map(count => articleProxy.updateItemById(count._id, { $set: { 'meta.comments': count.total_count } }).exec())
        )
