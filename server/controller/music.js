...
exports.list = async (ctx, next) => {
	...
	if (ctx._isAuthenticated) {
		const playListId = ctx.validateQuery('play_list_id').required('缺少歌单ID').notEmpty().val()
...
exports.url = async (ctx, next) => {
	const songId = ctx.validateParam('song_id').required('缺少歌曲ID').notEmpty().val()
...
exports.cover = async (ctx, next) => {
	const coverId = ctx.validateParam('cover_id').required('缺少封面ID').notEmpty().val()
